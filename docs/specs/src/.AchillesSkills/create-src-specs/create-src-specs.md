...
The system retrieves context from the `./docs/specs/` source directory. It iterates through the immediate children of this directory to identify relevant specification files. The filter includes only files that end in `.md` and start with the prefix `FDS`. It explicitly excludes the `src` and `tests` subdirectories, as well as any files or directories starting with `.` (hidden files). The module then reads and stores the content of all qualifying files.
...
### 6. File Generation
The module writes the generated files to the target directory `./docs/specs/src/`. For each parsed file, the system sanitizes the relative path to prevent directory traversal and resolves the full destination path. It recursively creates any necessary parent directories. Finally, it writes the content to the file, overwriting any existing version. The module operates immediately without a dry-run mode and does not create or update backlog files such as `specs_backlog.md`.
