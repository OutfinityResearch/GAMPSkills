...
There is one backlog for all specs: `./specs_backlog.md`. It is organized into primary sections, one per specification file. Inside each section you will find a short description of the spec, a status such as `ok`, `broken`, or `needs-info`, space for issues and proposed fixes, and a resolution field that records the user-approved solution to be applied. Everything is plain text and human-editable so approvals and updates are straightforward.
