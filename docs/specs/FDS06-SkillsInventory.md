...
## Outcomes
Together these skills provide a complete loop: initialize a project, draft global specs, deepen into technical specs, author test specs, review and backlog proposals, apply approved fixes, generate or regenerate code via the AchillesCLI code-generation skill, and document the resulting system. Actual JavaScript code (including tests) is handled by that dedicated AchillesCLI skill invoked explicitly by the user, using the FDS files as authoritative input. The backlogs at the project root keep authority with the user, while deterministic skills ensure every change to specs or documentation is deliberate, traceable, and regenerable.
