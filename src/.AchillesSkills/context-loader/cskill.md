...
- **options** (object, optional): Configuration options provided after `options:` as a JSON object.
...
- Optional parameters must be provided after `options:` as a JSON object.
...
- **include** (string[] or comma-separated string): Force-include these files before the first LLM call, regardless of filter. Default: none.
...
- "understand the backlog system" options: {"dir": "./src", "filter": "*.mjs", "maxDepth": 3, "include": ["package.json", "tsconfig.json"]}
